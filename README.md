...
- 所有校验以**回执**中钉死的长度与摘要为锚：与回执矛盾的 `meta.json` 被静默篡改时
  直接忽略，不影响裁决；回执自身内部矛盾时绝不报告健康。
...
- **封存后回执是唯一信任根**：`meta.json` 等其他持久描述若与回执矛盾（如被静默改成
  同长度错误文件的摘要），一律以回执为准——错误文件永远拿不到改写已封存分块的凭据，
  其修复请求被稳定拒绝且数据零改动；复核也绝不把与回执不一致的内容报为健康。
  重启、重复提交或旧会话首次复核，对同一矛盾状态的裁决始终一致。
